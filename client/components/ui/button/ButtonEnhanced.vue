...
<template>
  ...
  >
    ...
    <template v-if="icon && iconPlacement === 'right'">
      <template v-if="effect === 'expandIcon'">
        <div
          className="w-0 translate-x-[100%] -ml-2 opacity-0 transition-all duration-200 group-hover:w-5 group-hover:translate-x-0 group-hover:pl-2 group-hover:opacity-100"
        >
          <component :is="icon" />
        </div>
      </template>
